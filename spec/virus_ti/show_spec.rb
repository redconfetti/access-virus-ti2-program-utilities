...
RSpec.describe VirusTi::Show do
  describe ".build" do
    it "selects a program from a bank fixture by slot" do
      selection = described_class.build(fixture_path("ostirus/banks/redconfetti.syx"), slot: 1)

      expect(selection.label).to include("treefrogJM")
    ...
    it "selects a part from an arrangement fixture by slot" do
      selection = described_class.build(
        fixture_path("ostirus/arrangements/arcadia-arrangement.syx"),
        slot: 1
      )

      expect(selection.label).to include("Part 1")
      expect(selection.label).to include("Arkadia1-J")
      expect(selection.context).to eq(:arrangement)
    end

    it "requires a slot number" do
      expect do
        described_class.build(fixture_path("ostirus/banks/redconfetti.syx"), slot: nil)
  ...
  describe ".parameter_groups" do
    it "returns grouped parameters for all categories" do
      selection = described_class.build(fixture_path("ostirus/programs/organ-stab.syx"), slot: 1)
...
RSpec.describe VirusTi::Output::Formatter do
  describe ".render" do
    let(:selection) do
      VirusTi::Show.build(fixture_path("ostirus/programs/organ-stab.syx"), slot: 1)
    end
    ...
    it "renders text output with categories" do
      ...
      expect(text).to include("Organ Stab")
    ...
    it "renders csv output" do
      ...
      expect(csv).to include("Organ Stab").or include("Osc/Mixer")
    end
